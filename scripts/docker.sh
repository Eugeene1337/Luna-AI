...
case "$1" in
    build)
        docker build --platform linux/amd64 -t luna .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=luna)" ]; then
            echo "Container 'luna' is already running. Stopping it first."
            docker stop luna
            docker rm luna
        fi
    ...
    bash)
        ...
        if [ "$(docker ps -q -f name=luna)" ]; then
            docker exec -it luna bash
        else
            echo "Container 'luna' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
