...
export class Cache {
    ...
    constructor() {
        ...
        // Find the 'luna' folder in the filepath and adjust the cache directory path
        const lunaIndex = __dirname.indexOf("luna");
        if (lunaIndex !== -1) {
            const pathToluna = __dirname.slice(0, lunaIndex + 5); // include 'luna'
            this.cacheDir = path.join(pathToluna, "cache");
        } else {
