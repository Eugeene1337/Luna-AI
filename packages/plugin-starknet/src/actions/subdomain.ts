...
export default {
    ...
    handler: async (
        ...
        lunaLogger.log("Starting CREATE_SUBDOMAIN handler...");
        ...
        lunaLogger.debug("Transfer content:", content);

        // Validate transfer content
        if (!isSubdomainCreation(content)) {
            lunaLogger.error("Invalid content for CREATE_SUBDOMAIN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            lunaLogger.error("Error during subdomain transfer:", error);
            if (callback) {
                callback({
