...
export default {
    ...
    handler: async (
        ...
        lunaLogger.log("Starting SEND_TOKEN handler...");
        ...
        if (!isTransferContent(content)) {
            lunaLogger.error("Invalid content for TRANSFER_TOKEN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            lunaLogger.error("Error during token transfer:", error);
            if (callback) {
                callback({
