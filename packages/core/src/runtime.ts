...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            lunaLogger.error(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        lunaLogger.log("Registering service:", serviceType);
        ...
        lunaLogger.success(`Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        lunaLogger.info("Initializing AgentRuntime with options:", {
        ...
        lunaLogger.success("Agent ID", this.agentId);
        ...
        lunaLogger.info("Setting model provider...");
        lunaLogger.info("Model Provider Selection:", {
        ...
        lunaLogger.info("Selected model provider:", this.modelProvider);
        lunaLogger.info(
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            lunaLogger.error("Invalid model provider:", this.modelProvider);
            lunaLogger.error(
                "Available providers:",
        ...
        if (!this.serverUrl) {
            lunaLogger.warn("No serverUrl provided, defaulting to localhost");
        }
    ...
    async stop() {
      lunaLogger.debug('runtime::stop - character', this.character)
      ...
      for(const cStr in this.clients) {
        ...
        lunaLogger.log('runtime::stop - requesting', cStr, 'client stop for', this.character.name)
        c.stop()
      }
    ...
    registerAction(action: Action) {
        lunaLogger.success(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                lunaLogger.warn("No action found in the response content.");
                continue;
            }
            ...
            lunaLogger.success(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                lunaLogger.info("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                lunaLogger.error(`Action ${action.name} has no handler.`);
                continue;
            }
    ...
    async evaluate(
        ...
        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                lunaLogger.log("Evaluating", evaluator.name);
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            lunaLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            lunaLogger.log(`Room ${roomId} created successfully.`);
        }
    }
