...
import { lunaLogger } from "./index.ts";
...
export async function generateText({
    ...
    lunaLogger.log("Generating text...");

    lunaLogger.info("Generating text with options:", {
    ...
    lunaLogger.info("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.AKASH_CHAT_API: {
                lunaLogger.debug("Initializing OpenAI model.");
                ...
                lunaLogger.debug("Received response from OpenAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                lunaLogger.debug("Received response from Google model.");
                break;
            }

            case ModelProviderName.ANTHROPIC: {
                lunaLogger.debug("Initializing Anthropic model.");
                ...
                lunaLogger.debug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                lunaLogger.debug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                lunaLogger.debug("Initializing Grok model.");
                ...
                lunaLogger.debug("Received response from Grok model.");
                break;
            }
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                lunaLogger.debug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                lunaLogger.debug("Initializing RedPill model.");
                ...
                lunaLogger.debug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                lunaLogger.debug("Initializing OpenRouter model.");
                const serverUrl = models[provider].endpoint;
                ...
                lunaLogger.debug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    lunaLogger.debug("Initializing Ollama model.");
                ...
                lunaLogger.debug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                lunaLogger.debug("Initializing Heurist model.");
                ...
                lunaLogger.debug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                lunaLogger.debug("Initializing GAIANET model.");
                ...
                lunaLogger.debug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                lunaLogger.debug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                lunaLogger.debug("Initializing Galadriel model.");
                ...
                lunaLogger.debug("Received response from Galadriel model.");
                break;
            }

            case ModelProviderName.VENICE: {
                lunaLogger.debug("Initializing Venice model.");
                ...
                lunaLogger.debug("Received response from Venice model.");
                break;
            }

            default: {
                const errorMessage = `Unsupported provider: ${provider}`;
                lunaLogger.error(errorMessage);
        ...
        lunaLogger.error("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            lunaLogger.debug("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                lunaLogger.debug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                lunaLogger.debug("generateShouldRespond no response");
            }
        } catch (error) {
            lunaLogger.error("Error in generateShouldRespond:", error);
            if (
        ...
        lunaLogger.log(`Retrying in ${retryDelay}ms...`);
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            lunaLogger.error("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        lunaLogger.error("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            lunaLogger.error("Error in generateTextArray:", error);
        }
...
export async function generateObjectDeprecated({
    ...
    if (!context) {
        lunaLogger.error("generateObjectDeprecated context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            lunaLogger.error("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        lunaLogger.error("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            lunaLogger.log("Generating message response..");
            ...
            if (!parsedContent) {
                lunaLogger.debug("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            lunaLogger.error("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            lunaLogger.debug("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    lunaLogger.info("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        lunaLogger.error("Missing URL in image data:", image);
            ...
            lunaLogger.debug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model, {
                ...
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        lunaLogger.info(update.logs.map((log) => log.message));
                    }
                },
            });
    ...
    while (true) {
        try {
            ...
            if (actions) {
                ...
                lunaLogger.debug("generateTweetActions no valid response");
            }
        } catch (error) {
            lunaLogger.error("Error in generateTweetActions:", error);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
