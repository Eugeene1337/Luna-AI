...
async function get(
    ...
    if (!message?.content?.text) {
        lunaLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    lunaLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        lunaLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    lunaLogger.debug("Preprocessing text:", {
        input: content,
        length: content?.length,
    });

    if (!content || typeof content !== "string") {
        lunaLogger.warn("Invalid input for preprocessing");
        return "";
    }
