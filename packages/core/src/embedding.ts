...
import lunaLogger from "./logger.ts";
...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            lunaLogger.error("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        lunaLogger.error("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string) {
    lunaLogger.debug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        lunaLogger.warn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function getLocalEmbedding(input: string): Promise<number[]> {
        lunaLogger.debug("DEBUG - Inside getLocalEmbedding function");
        ...
        try {
            const moduleImports = await Promise.all([
                ...
                (async () => {
                    try {
                        ...
                        lunaLogger.error("Failed to load fastembed.");
            ...
            function getRootPath() {
                ...
                if (rootPath.includes("/luna/")) {
                    return rootPath.split("/luna/")[0] + "/luna/";
                }
            ...
            lunaLogger.debug("Initializing BGE embedding model...");
            ...
            lunaLogger.debug("Generating embedding for input:", {
                inputLength: input.length,
            ...
            lunaLogger.debug("Raw embedding from BGE:", {
            ...
            lunaLogger.debug("Processed embedding:", {
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            lunaLogger.log("No input to retrieve cached embedding for");
            return null;
        }
