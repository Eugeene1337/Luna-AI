...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            lunaLogger.error("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            lunaLogger.debug("Memory already exists, skipping");
            return;
        }

        lunaLogger.log("Creating Memory", memory.id, memory.content.text);
