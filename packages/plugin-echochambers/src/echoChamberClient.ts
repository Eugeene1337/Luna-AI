import { lunaLogger, IAgentRuntime } from "@luna/core";
import {
    ChatMessage,
    ChatRoom,
    EchoChamberConfig,
...
export class EchoChamberClient {
    ...
    public async setWatchedRoom(roomId: string): Promise<void> {
        try {
            ...
            lunaLogger.success(`Now watching room: ${room.name}`);
        } catch (error) {
            lunaLogger.error("Error setting watched room:", error);
            throw error;
        }
    }
    ...
    private async retryOperation<T>(
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                lunaLogger.warn(`Retrying operation in ${delay}ms...`);
    ...
    public async start(): Promise<void> {
        lunaLogger.log("🚀 Starting EchoChamber client...");
        try {
            ...
            lunaLogger.error("❌ Failed to start EchoChamber client:", error);
            throw error;
        }
    }
    ...
    public async stop(): Promise<void> {
        ...
        lunaLogger.log("Stopping EchoChamber client...");
    }
    ...
    public async listRooms(tags?: string[]): Promise<ChatRoom[]> {
        try {
            ...
            lunaLogger.error("Error listing rooms:", error);
            throw error;
        }
    }
