...
export class FarcasterPostManager {
    ...
    private async generateNewCast() {
        lunaLogger.info("Generating new cast");
        try {
            ...
            if (this.runtime.getSetting("FARCASTER_DRY_RUN") === "true") {
                lunaLogger.info(`Dry run: would have cast: ${content}`);
                return;
            }
            ...
            try {
                ...
                lunaLogger.error("Error sending cast:", error);
            }
        } catch (error) {
            lunaLogger.error("Error generating new cast:", error);
        }
    }
}
