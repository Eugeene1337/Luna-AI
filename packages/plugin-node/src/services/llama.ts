...
export class LlamaService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        lunaLogger.info("Initializing LlamaService...");
    ...
    private async ensureInitialized() {
        if (!this.modelInitialized) {
            ...
            lunaLogger.info("Model already initialized");
        }
    }

    async initializeModel() {
        try {
            lunaLogger.info("Checking model file...");
            ...
            lunaLogger.info("Initializing Llama instance...");
            this.llama = await getLlama({
                gpu: hasCUDA ? "cuda" : undefined,
            });

            lunaLogger.info("Creating JSON schema grammar...");
            ...
            lunaLogger.info("Loading model...");
            this.model = await this.llama.loadModel({
                modelPath: this.modelPath,
            });

            lunaLogger.info("Creating context and sequence...");
            this.ctx = await this.model.createContext({ contextSize: 8192 });
            this.sequence = this.ctx.getSequence();

            this.modelInitialized = true;
            lunaLogger.success("Model initialization complete");
            this.processQueue();
        } catch (error) {
            lunaLogger.error(
    ...
    async checkModel() {
        if (!fs.existsSync(this.modelPath)) {
            lunaLogger.info("Model file not found, starting download...");
                    ...
                    https
                        .get(url, (response) => {
                            ...
                            response.on("data", (chunk) => {
                                ...
                                lunaLogger.progress(progressString);
                            });

                            file.on("finish", () => {
                                file.close();
                                lunaLogger.progress(""); // Clear the progress line
                                lunaLogger.success("Model download complete");
                                resolve();
                            });
            ...
            lunaLogger.warn("Model already exists.");
        }
    }
    ...
    async completion(prompt: string, runtime: IAgentRuntime): Promise<string> {
        try {
            ...
            lunaLogger.error("Error in completion:", error);
            throw error;
        }
    }
    ...
    async embedding(text: string, runtime: IAgentRuntime): Promise<number[]> {
        try {
            ...
            lunaLogger.error("Error in embedding:", error);
            throw error;
        }
    }
    ...
    private async getCompletionResponse(
        ...
        for await (const token of this.sequence.evaluate(tokens, {
            ...
            if ([...stop].some((s) => current.includes(s))) {
                lunaLogger.info("Stop sequence found");
                break;
            }
            ...
            if (useGrammar) {
                if (current.replaceAll("\n", "").includes("}```")) {
                    lunaLogger.info("JSON block found");
                    break;
                }
            }
            if (responseTokens.length > max_tokens) {
                lunaLogger.info("Max tokens reached");
                break;
            }
        }
        ...
        if (useGrammar) {
            ...
            try {
                ...
                lunaLogger.error("Error parsing JSON:", error);
            }
        } else {
            await this.sequence.clearHistory();
            return response;
