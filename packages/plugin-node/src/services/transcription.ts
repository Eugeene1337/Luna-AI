...
import { Service, ServiceType } from "@luna/core";
    ...
    implements ITranscriptionService
{
    ...
    private async convertAudio(inputBuffer: ArrayBuffer): Promise<Buffer> {
        ...
        try {
            ...
            lunaLogger.log("Input audio info:", stream);
            ...
            lunaLogger.log("FFmpeg command:", ffmpegCommand);
            ...
            lunaLogger.error("Error converting audio:", error);
            throw error;
        }
    }
    ...
    private async saveDebugAudio(audioBuffer: ArrayBuffer, prefix: string) {
        ...
        lunaLogger.log(`Debug audio saved: ${filePath}`);
    }
    ...
    private async transcribeWithOpenAI(
        ...
        lunaLogger.log("Transcribing audio with OpenAI...");
        ...
        try {
            ...
            lunaLogger.log(`OpenAI speech to text result: "${trimmedResult}"`);
            ...
            if (error.response) {
                lunaLogger.error("Response data:", error.response.data);
                lunaLogger.error("Response status:", error.response.status);
                lunaLogger.error("Response headers:", error.response.headers);
            } else if (error.request) {
                lunaLogger.error("No response received:", error.request);
            } else {
                lunaLogger.error("Error setting up request:", error.message);
            }
            return null;
        }
    }
    ...
    public async transcribeLocally(
        ...
        try {
            lunaLogger.log("Transcribing audio locally...");
            ...
            lunaLogger.debug(`Temporary WAV file created: ${tempWavFile}`);
            ...
            if (!output || output.length < 5) {
                lunaLogger.log("Output is null or too short, returning null");
                return null;
            }
            return output;
        } catch (error) {
