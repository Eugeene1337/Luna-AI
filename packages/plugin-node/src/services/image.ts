import { lunaLogger, models } from "@luna/core";
    ...
    implements IImageDescriptionService
{
    ...
    private async initializeLocalModel(): Promise<void> {
        ...
        lunaLogger.info("Downloading Florence model...");
        ...
        lunaLogger.success("Florence model downloaded successfully");

        lunaLogger.info("Downloading processor...");
        this.processor = (await AutoProcessor.from_pretrained(
            this.modelId
        )) as Florence2Processor;

        lunaLogger.info("Downloading tokenizer...");
        this.tokenizer = await AutoTokenizer.from_pretrained(this.modelId);
        lunaLogger.success("Image service initialization complete");
    }
    ...
    private async recognizeWithOpenAI(
        ...
        try {
            ...
            lunaLogger.error("Error in recognizeWithOpenAI:", error);
            throw error;
        }
    }
    ...
    private async processImage(
        ...
        lunaLogger.log("Processing image:", imageUrl);
        const isGif = imageUrl.toLowerCase().endsWith(".gif");
        let imageToProcess = imageUrl;

        try {
            if (isGif) {
                lunaLogger.log("Extracting first frame from GIF");
                const { filePath } =
                    await this.extractFirstFrameFromGif(imageUrl);
            ...
            lunaLogger.log("Generating image description");
            ...
            lunaLogger.error("Error processing image:", error);
            throw error;
        } finally {
