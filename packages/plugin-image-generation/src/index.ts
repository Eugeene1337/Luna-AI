...
import { generateImage } from "@luna/core";
...
const imageGeneration: Action = {
    ...
    handler: async (
        ...
        lunaLogger.log("Composing state for message:", message);
        state = (await runtime.composeState(message)) as State;
        const userId = runtime.agentId;
        lunaLogger.log("User ID:", userId);

        const imagePrompt = message.content.text;
        lunaLogger.log("Image prompt received:", imagePrompt);

        // TODO: Generate a prompt for the image

        const res: { image: string; caption: string }[] = [];

        lunaLogger.log("Generating image with prompt:", imagePrompt);
        ...
        if (images.success && images.data && images.data.length > 0) {
            ...
            for (let i = 0; i < images.data.length; i++) {
                ...
                lunaLogger.log(`Processing image ${i + 1}:`, filename);
                ...
                try {
                    ...
                    lunaLogger.error("Caption generation failed, using default caption:", error);
                }*/
            ...
            lunaLogger.error("Image generation failed or returned no data.");
        }
    },
    examples: [
