...
export class FlowWalletProvider implements IFlowSigner, IFlowScriptExecutor {
    ...
    async syncAccountInfo() {
        ...
        lunaLogger.debug("Flow account info synced", {
...
function getSignerAddress(runtime: IAgentRuntime): string {
    ...
    if (!signerAddr) {
        lunaLogger.error("No signer address");
    ...
    get: async (
        ...
        try {
            ...
            if (!info || info?.address !== walletProvider.address) {
                lunaLogger.error("Invalid account info");
                return null;
            }
            ...
            lunaLogger.error("Error in Flow wallet provider:", error.message);
            return null;
        }
    },
};
