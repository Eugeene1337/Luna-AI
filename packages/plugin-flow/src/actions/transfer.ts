...
function isTransferContent(
    ...
    lunaLogger.log("Content for transfer", content);
    return (
        (!content.token ||
...
export class TransferAction {
    ...
    async processMessages(
        ...
        lunaLogger.debug("Recommendations", recommendations);
        ...
        if (!isTransferContent(runtime, content)) {
            lunaLogger.error("Invalid content for SEND_COIN action.");
            throw new Exception(50100, "Invalid transfer content");
        }

        // Check if the content is matched
        if (!content.matched) {
            lunaLogger.error("Content does not match the transfer template.");
            throw new Exception(
                50100,
    ...
    async transfer(
        ...
        lunaLogger.log("Starting Flow Plugin's SEND_COIN handler...");
        ...
        if (totalBalance < amount) {
            lunaLogger.error("Insufficient balance to transfer.");
            if (callback) {
                callback({
        ...
        try {
            ...
            lunaLogger.log(`${logPrefix} Sent transaction: ${resp.txid}`);
            ...
            lunaLogger.error("Error in sending transaction:", e.message);
            if (callback) {
                callback({
        ...
        lunaLogger.log("Completed Flow Plugin's SEND_COIN handler.");
...
export const transferAction = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ...
        try {
            ...
            lunaLogger.error("Failed to sync account info");
            return false;
        }
        return true;
    },
    handler: async (
        ...
        try {
            ...
            lunaLogger.error("Error in processing messages:", err.message);
            if (callback) {
                callback({
                    text:
