...
export class LensPostManager {
    ...
    private async generateNewPublication() {
        lunaLogger.info("Generating new publication");
        try {
            ...
            if (this.runtime.getSetting("LENS_DRY_RUN") === "true") {
                lunaLogger.info(`Dry run: would have posted: ${content}`);
                return;
            }
            ...
            try {
                ...
                lunaLogger.info(`[Lens Client] Published ${publication.id}`);
                ...
                lunaLogger.error("Error sending publication:", error);
            }
        } catch (error) {
            lunaLogger.error("Error generating new publication:", error);
        }
    }
}
