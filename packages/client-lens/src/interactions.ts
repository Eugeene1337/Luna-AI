...
export class LensInteractionManager {
    ...
    private async handleInteractions() {
        lunaLogger.info("Handle Lens interactions");
    ...
    private async handlePublication({
        ...
        if (publication.by.id === agent.id) {
            lunaLogger.info("skipping cast from bot itself", publication.id);
            return;
        }

        if (!memory.content.text) {
            lunaLogger.info("skipping cast with no text", publication.id);
