...
const generateVideo = async (prompt: string, runtime: IAgentRuntime) => {
    ...
    try {
        lunaLogger.log("Starting video generation with prompt:", prompt);
        ...
        if (!response.ok) {
            ...
            lunaLogger.error("Luma API error:", {
        ...
        while (status !== "completed" && status !== "failed") {
            ...
            if (!statusResponse.ok) {
                ...
                lunaLogger.error("Status check error:", {
            ...
            lunaLogger.log("Status check response:", statusData);
        ...
        lunaLogger.error("Video generation error:", error);
        return {
            success: false,
...
const videoGeneration: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        lunaLogger.log("Validating video generation action");
        const lumaApiKey = runtime.getSetting("LUMA_API_KEY");
        lunaLogger.log("LUMA_API_KEY present:", !!lumaApiKey);
        return !!lumaApiKey;
    },
    handler: async (
        ...
        lunaLogger.log("Video generation request:", message);
        ...
        lunaLogger.log("Video prompt:", videoPrompt);
        ...
        try {
            ...
            lunaLogger.error(`Failed to generate video. Error: ${error}`);
            callback({
