...
class GetIPDetailsAction {
    async getIPDetails(
        ...
        lunaLogger.log("Fetching from", `${API_URL}/${RESOURCE_TYPE.ASSET}`);
...
export const getIPDetailsAction = {
    ...
    handler: async (
        ...
        lunaLogger.log("Starting GET_IP_DETAILS handler...");
        ...
        try {
            ...
            lunaLogger.error("Error fetching IP details:", e.message);
            callback?.({
