...
export async function getResource(
    ...
    try {
        ...
        if (res.ok) {
            lunaLogger.log("Response is ok");
            return res.json();
        } else {
            lunaLogger.log("Response is not ok");
            lunaLogger.log(JSON.stringify(res));
...
export async function listResource(
    ...
    try {
        ...
        lunaLogger.log(`Calling Story API ${resourceName}`);
        lunaLogger.log(`STORY_API_KEY: ${API_KEY}`);
        lunaLogger.log(`API_URL: ${API_URL}`);
        lunaLogger.log(`API_VERSION: ${API_VERSION}`);
        lunaLogger.log(`_options: ${JSON.stringify(_options)}`);
        ...
        lunaLogger.log("List resource Error");
        console.error(error);
    }
}
