...
import { models } from "@luna/core";
import { getActorDetails } from "@luna/core";
import {
    Action,
    ActionExample,
    Content,
...
const summarizeAction: Action = {
    ...
    handler: async (
        ...
        if (!dateRange) {
            lunaLogger.error("Couldn't determine date range from message");
            callbackData.text =
...
Here is a detailed summary of the conversation between ${userName} and ${runtime.character.name}:\n\n${currentSummary.trim()}`;
            ...
            if (summaryContent.length > 1000) {
                ...
                lunaLogger.debug("Uploading summary file to Slack...");
                ...
                try {
                    ...
                    lunaLogger.error("Error uploading summary file:", error);
            ...
            lunaLogger.error("Error in summary generation:", error);
            callbackData.text =
