...
import { lunaLogger, IAgentRuntime, settings } from "@luna/core";
import fs from "fs";
    ...
    if (!storedSeed || !storedWalletId) {
        ...
        try {
            ...
            if (walletIDSave && seedSave) {
                lunaLogger.log("Successfully updated character secrets.");
            } else {
            ...
            lunaLogger.error("Error updating character secrets:", error);
            throw error;
        }

        // Logging wallet creation
        lunaLogger.log("Created and stored new wallet:", walletAddress);
    } else {
        ...
        lunaLogger.log("Imported existing wallet for network:", networkId);
...
export async function executeTradeAndCharityTransfer(
    ...
    lunaLogger.log("Wallet initialized:", {
        network,
    ...
    if (charityAddress && charityAmount > 0) {
        ...
        lunaLogger.log("Charity Transfer successful:", {
    ...
    lunaLogger.log("Trade initiated:", trade.toString());
    await trade.wait();
    lunaLogger.log("Trade completed successfully:", trade.toString());
...
export async function appendTradeToCsv(trade: Trade) {
    try {
        ...
        lunaLogger.log("Writing trade to CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        lunaLogger.log("Trade written to CSV successfully.");
    } catch (error) {
        lunaLogger.error("Error writing trade to CSV:", error);
    }
}
...
export async function appendTransactionsToCsv(transactions: Transaction[]) {
    try {
        ...
        lunaLogger.log("Writing transactions to CSV:", formattedTransactions);
        await csvWriter.writeRecords(formattedTransactions);
        lunaLogger.log("All transactions written to CSV successfully.");
    } catch (error) {
        lunaLogger.error("Error writing transactions to CSV:", error);
    }
}
...
export async function appendWebhooksToCsv(webhooks: Webhook[]) {
    try {
        ...
        if (!fs.existsSync(webhookCsvFilePath)) {
            lunaLogger.warn("CSV file not found. Creating a new one.");
            const csvWriter = createArrayCsvWriter({
            ...
            lunaLogger.log("New CSV file created with headers.");
        }
        const csvWriter = createArrayCsvWriter({
            path: webhookCsvFilePath,
            header: [
        ...
        lunaLogger.log("Writing webhooks to CSV:", formattedWebhooks);
        await csvWriter.writeRecords(formattedWebhooks);
        lunaLogger.log("All webhooks written to CSV successfully.");
    } catch (error) {
        lunaLogger.error("Error writing webhooks to CSV:", error);
    }
}
...
export async function updateCharacterSecrets(
    ...
    try {
        ...
        if (!fs.existsSync(characterFilePath)) {
            lunaLogger.error("Character file not found:", characterFilePath);
            return false;
        }
...
export async function executeTransferAndCharityTransfer(
    ...
    lunaLogger.log("Initiating transfer:", transferDetails);
    const transfer = await wallet.createTransfer(transferDetails);
    lunaLogger.log("Transfer initiated:", transfer.toString());
    await transfer.wait();
...
export async function executeTransfer(
    ...
    try {
        ...
        lunaLogger.error("Error executing transfer:", error);
    }
    return transfer;
}
