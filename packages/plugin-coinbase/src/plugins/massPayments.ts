...
export const massPayoutProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        try {
            ...
            lunaLogger.log("Reading CSV file from:", csvFilePath);
            ...
            lunaLogger.log("Parsed CSV records:", records);
            lunaLogger.log("Current Balances:", balances);
            lunaLogger.log("Last Transactions:", transactions);
            ...
            lunaLogger.error("Error in massPayoutProvider:", error);
...
async function executeMassPayout(
    ...
    try {
        ...
        lunaLogger.error("Error initializing sending wallet:", error);
        throw error;
    }
    for (const address of receivingAddresses) {
        lunaLogger.log("Processing payout for address:", address);
        if (address) {
            try {
                ...
                lunaLogger.log("Wallet balance for asset:", {
                    assetId,
                    walletBalance,
                });
                ...
                if (walletBalance.lessThan(transferAmount)) {
                    ...
                    lunaLogger.error(insufficientFunds);
            ...
            lunaLogger.log("Skipping invalid or empty address.");
            transactions.push({
    ...
    try {
        ...
        lunaLogger.error("Error during charity transfer:", error);
        transactions.push({
            address: charityAddress,
    ...
    lunaLogger.log("Finished processing mass payouts.");
    return transactions;
}
...
export const sendMassPayoutAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        lunaLogger.log("Validating runtime and message...");
        return (
            !!(
    ...
    handler: async (
        ...
        lunaLogger.log("Starting SEND_MASS_PAYOUT handler...");
        try {
            Coinbase.configure({
                apiKeyName:
                    runtime.getSetting("COINBASE_API_KEY") ??
            ...
            if (
                ...
                lunaLogger.error("Missing or invalid input parameters:", {
                    network,
                    receivingAddresses,
                    transferAmount,
                    assetId,
...
- Asset ID is valid.`,
            ...
            lunaLogger.log("◎ Starting mass payout...");
...
Check the CSV file for full details.`,
            ...
            lunaLogger.error("Error during mass payouts:", error);
            callback(
