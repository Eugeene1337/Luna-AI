import { AwsS3Service } from "@luna/plugin-node";
import {
    composeContext,
    lunaLogger,
    generateImage,
...
import {
    saveBase64Image,
    saveHeuristImage,
} from "@luna/plugin-image-generation";
...
export async function createNFTMetadata({
    ...
    lunaLogger.log("NFT Prompt:", nftPrompt);
    if (images.success && images.data && images.data.length > 0) {
        const image = images.data[0];
        const filename = `${tokenId}`;
        if (image.startsWith("http")) {
            lunaLogger.log("Generating image url:", image);
        }
...
export async function createNFT({
    ...
    if (nftInfo) {
        ...
        lunaLogger.log("NFT ID:", nftAddressRes.address);
        return {
            network: "solana",
