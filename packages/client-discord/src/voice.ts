...
export class AudioMonitor {
    ...
    constructor(
        ...
        this.readable.on("end", () => {
            lunaLogger.log("AudioMonitor ended");
            this.ended = true;
        ...
        this.readable.on("speakingStopped", () => {
            ...
            lunaLogger.log("Speaking stopped");
            if (this.lastFlagged < 0) return;
            callback(this.getBufferFromStart());
        });
        this.readable.on("speakingStarted", () => {
            if (this.ended) return;
            onStart();
            lunaLogger.log("Speaking started");
            this.reset();
        });
    }
...
export class VoiceManager extends EventEmitter {
        ...
        try {
            ...
            connection.on("stateChange", async (oldState, newState) => {
                ...
                if (newState.status === VoiceConnectionStatus.Disconnected) {
                    lunaLogger.log("Handling disconnection...");
                    ...
                    try {
                        ...
                        lunaLogger.log("Reconnecting to channel...");
                    } catch (e) {
            ...
            connection.on("error", (error) => {
                lunaLogger.log("Voice connection error:", error);
            ...
            if (me?.voice && me.permissions.has("DeafenMembers")) {
                try {
                    ...
                    lunaLogger.log("Failed to modify voice state:", error);
            ...
            lunaLogger.log("Failed to establish voice connection:", error);
            connection.destroy();
            this.connections.delete(channel.id);
            throw error;
        }
    ...
    async debouncedProcessTranscription(
        ...
        if (this.activeAudioPlayer?.state?.status === "idle") {
            lunaLogger.log("Cleaning up idle audio player.");
            this.cleanupAudioPlayer(this.activeAudioPlayer);
        }
    ...
    private async _shouldIgnore(message: Memory): Promise<boolean> {
        ...
        lunaLogger.debug("message.content: ", message.content);
