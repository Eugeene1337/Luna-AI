...
export class DiscordClient extends EventEmitter {
    ...
    async stop() {
        try {
            ...
            lunaLogger.error("client-discord instance stop err", e);
        }
    }

    private async onClientReady(readyClient: { user: { tag: any; id: any } }) {
        lunaLogger.success(`Logged in as ${readyClient.user?.tag}`);
        ...
        try {
            ...
            lunaLogger.success("Slash commands registered");
        } catch (error) {
        ...
        lunaLogger.success("Use this URL to add the bot to your server:");
        lunaLogger.success(
    ...
    async handleReactionAdd(reaction: MessageReaction, user: User) {
        try {
            lunaLogger.log("Reaction added");

            // Early returns
            if (!reaction || !user) {
                lunaLogger.warn("Invalid reaction or user");
                return;
            }
            ...
            if (!userIdUUID || !roomId) {
                lunaLogger.error("Invalid user ID or room ID", {
                    userIdUUID,
                    roomId,
                });
                return;
            ...
            try {
                ...
                lunaLogger.debug("Reaction memory created", {
                ...
                if (error.code === "23505") {
                    // Duplicate key error
                    lunaLogger.warn("Duplicate reaction memory, skipping", {
                        reactionId: reactionUUID,
                    });
                    return;
                }
            ...
            lunaLogger.error("Error handling reaction:", error);
        }
    }

    async handleReactionRemove(reaction: MessageReaction, user: User) {
        lunaLogger.log("Reaction removed");
        // if (user.bot) return;
...
export const DiscordClientInterface: lunaClient = {
    start: async (runtime: IAgentRuntime) => new DiscordClient(runtime),
    stop: async (runtime: IAgentRuntime) => {
        try {
            // stop it
            lunaLogger.log("Stopping discord client", runtime.agentId);
            await runtime.clients.discord.stop();
        } catch (e) {
            lunaLogger.error("client-discord interface stop error", e);
        }
    },
};
