    ...
    implements IDatabaseCacheAdapter
{
    ...
    constructor(connectionConfig: any) {
        ...
        this.pool.on("error", (err) => {
            lunaLogger.error("Unexpected pool error", err);
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries) {
                    ...
                    lunaLogger.error("Max retry attempts reached:", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
    ...
    private async handlePoolError(error: Error) {
        lunaLogger.error("Pool error occurred, attempting to reconnect", {
            error: error.message,
        });
        ...
        try {
            ...
            lunaLogger.success("Pool reconnection successful");
        } catch (reconnectError) {
            lunaLogger.error("Failed to reconnect pool", {
                error:
    ...
    async testConnection(): Promise<boolean> {
        ...
        try {
            ...
            lunaLogger.error("Database connection test failed:", error);
            throw new Error(
    ...
    async cleanup(): Promise<void> {
        try {
            await this.pool.end();
            lunaLogger.info("Database pool closed");
        } catch (error) {
            lunaLogger.error("Error closing database pool:", error);
        }
    }
    ...
    async getAccountById(userId: UUID): Promise<Account | null> {
        return this.withDatabase(async () => {
            ...
            if (rows.length === 0) {
                lunaLogger.debug("Account not found:", { userId });
                return null;
            }

            const account = rows[0];
            // lunaLogger.debug("Account retrieved:", {
            //     userId,
    ...
    async createAccount(account: Account): Promise<boolean> {
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Account created successfully:", {
                    accountId,
                });
                return true;
            } catch (error) {
                lunaLogger.error("Error creating account:", {
                    error:
    ...
    async getActorById(params: { roomId: UUID }): Promise<Actor[]> {
        return this.withDatabase(async () => {
            ...
            lunaLogger.debug("Retrieved actors:", {
            ...
            return rows.map((row) => {
                try {
                    ...
                    lunaLogger.warn("Failed to parse actor details:", {
                        actorId: row.id,
                        error:
                            error instanceof Error
                                ? error.message
            ...
            lunaLogger.error("Failed to get actors:", {
                roomId: params.roomId,
                error: error.message,
            });
    ...
    async createMemory(memory: Memory, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            lunaLogger.debug("PostgresAdapter createMemory:", {
                memoryId: memory.id,
    ...
    async getMemories(params: {
        ...
        return this.withDatabase(async () => {
            ...
            lunaLogger.debug("Fetching memories:", {
                roomId: params.roomId,
                tableName: params.tableName,
                unique: params.unique,
                agentId: params.agentId,
    ...
    async updateGoal(goal: Goal): Promise<void> {
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.error("Failed to update goal:", {
                    goalId: goal.id,
                    error:
                        error instanceof Error ? error.message : String(error),
                    status: goal.status,
    ...
    async removeGoal(goalId: UUID): Promise<void> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Goal removal attempt:", {
                    goalId,
                    removed: result?.rowCount ?? 0 > 0,
                });
            } catch (error) {
                lunaLogger.error("Failed to remove goal:", {
                    goalId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                if (checkResult.rowCount === 0) {
                    lunaLogger.warn("No room found to remove:", { roomId });
                ...
                await client.query("ROLLBACK");
                lunaLogger.error("Failed to remove room:", {
                    roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async createRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Relationship created successfully:", {
                    relationshipId,
                    userA: params.userA,
                    userB: params.userB,
                });
                ...
                if ((error as { code?: string }).code === "23505") {
                    // Unique violation
                    lunaLogger.warn("Relationship already exists:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
                    ...
                    lunaLogger.error("Failed to create relationship:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
    ...
    async getRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                if (rows.length > 0) {
                    lunaLogger.debug("Relationship found:", {
                ...
                lunaLogger.debug("No relationship found between users:", {
                    userA: params.userA,
                    userB: params.userB,
                });
                return null;
            } catch (error) {
                lunaLogger.error("Error fetching relationship:", {
                    userA: params.userA,
                    userB: params.userB,
                    error:
                        error instanceof Error ? error.message : String(error),
    ...
    async getRelationships(params: { userId: UUID }): Promise<Relationship[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Retrieved relationships:", {
                ...
                lunaLogger.error("Failed to fetch relationships:", {
                    userId: params.userId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCachedEmbeddings(opts: {
        ...
        return this.withDatabase(async () => {
            try {
                lunaLogger.debug("Fetching cached embeddings:", {
                ...
                lunaLogger.debug("Retrieved cached embeddings:", {
                    count: rows.length,
                    tableName: opts.query_table_name,
                    matchCount: opts.query_match_count,
                });
                ...
                lunaLogger.error("Error in getCachedEmbeddings:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    tableName: opts.query_table_name,
                    fieldName: opts.query_field_name,
    ...
    async log(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Log entry created:", {
                    logId,
                    type: params.type,
                    roomId: params.roomId,
                    userId: params.userId,
                ...
                lunaLogger.error("Failed to create log entry:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    type: params.type,
                    roomId: params.roomId,
    ...
    async searchMemoriesByEmbedding(
        ...
        return this.withDatabase(async () => {
            lunaLogger.debug("Incoming vector:", {
    ...
    async getActorDetails(params: { roomId: string }): Promise<Actor[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.debug("Retrieved actor details:", {
                    roomId: params.roomId,
                ...
                lunaLogger.error("Failed to fetch actor details:", {
                    roomId: params.roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCache(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                lunaLogger.error("Error fetching cache", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    key: params.key,
                    agentId: params.agentId,
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    lunaLogger.error("Error setting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    lunaLogger.error("Error deleting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
