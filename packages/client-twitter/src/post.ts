...
import { postActionResponseFooter } from "@luna/core";
import { generateTweetActions } from "@luna/core";
import { IImageDescriptionService, ServiceType } from "@luna/core";
...
export class TwitterPostClient {
    ...
    async start(postImmediately: boolean = false) {
        ...
        const generateNewTweetLoop = async () => {
            ...
            lunaLogger.log(`Next tweet scheduled in ${randomMinutes} minutes`);
        };
        ...
        const processActionsLoop = async () => {
            ...
            while (!this.stopProcessingActions) {
                try {
                    ...
                    if (results) {
                        lunaLogger.log(`Processed ${results.length} tweets`);
                        lunaLogger.log(
        ...
        if (enableActionProcessing) {
            ...
            lunaLogger.log("Action processing loop disabled by configuration");
        }
        generateNewTweetLoop();
    }
    ...
    private async generateNewTweet() {
        lunaLogger.log("Generating new tweet");
        ...
        try {
            ...
            lunaLogger.debug("generate post prompt:\n" + context);
            ...
            try {
                lunaLogger.log(`Posting new tweet:\n ${cleanedContent}`);
                ...
                lunaLogger.error("Error sending tweet:", error);
            }
        } catch (error) {
            lunaLogger.error("Error generating new tweet:", error);
        }
    }
    ...
    private async generateTweetContent(
        ...
        try {
            ...
            lunaLogger.debug("Response is not JSON, treating as plain text");
        }
    ...
    private async processTweetActions() {
        if (this.isProcessing) {
            lunaLogger.log("Already processing tweet actions, skipping");
            return null;
        }
        ...
        try {
            ...
            lunaLogger.log("Processing tweet actions");
                ...
                try {
                    ...
                    if (actionResponse.like) {
                        try {
                            ...
                            lunaLogger.log(`Liked tweet ${tweet.id}`);
                        } catch (error) {
                            lunaLogger.error(
                    ...
                    if (actionResponse.retweet) {
                        try {
                            ...
                            lunaLogger.log(`Retweeted tweet ${tweet.id}`);
                        } catch (error) {
                            lunaLogger.error(
            ...
            lunaLogger.error("Error in processTweetActions:", error);
            throw error;
        } finally {
    ...
    private async handleTextOnlyReply(
        ...
        try {
            ...
            const imageDescriptions = [];
            if (tweet.photos?.length > 0) {
                lunaLogger.log("Processing images in tweet for context");
                for (const photo of tweet.photos) {
                    const description = await this.runtime
                        .getService<IImageDescriptionService>(
                            ServiceType.IMAGE_DESCRIPTION
            ...
            if (tweet.quotedStatusId) {
                try {
                    ...
                    lunaLogger.error("Error fetching quoted tweet:", error);
                }
            }
            ...
            if (!replyText) {
                lunaLogger.error("Failed to generate valid reply content");
                return;
            }

            lunaLogger.debug("Final reply text to be sent:", replyText);
            ...
            if (body?.data?.create_tweet?.tweet_results?.result) {
                lunaLogger.log("Successfully posted reply tweet");
                ...
                lunaLogger.error("Tweet reply creation failed:", body);
            }
        } catch (error) {
            lunaLogger.error("Error in handleTextOnlyReply:", error);
        }
    }
