...
export class ClientBase extends EventEmitter {
    ...
    async init() {
        ...
        if (cachedCookies) {
            lunaLogger.info("Using cached cookies");
            await this.setCookiesFromArray(cachedCookies);
        }

        lunaLogger.log("Waiting for Twitter login");
        while (retries > 0) {
            try {
                ...
                if (await this.twitterClient.isLoggedIn()) {
                    lunaLogger.info("Successfully logged in.");
                    if (!cachedCookies) {
                        lunaLogger.info("Caching cookies");
                ...
                lunaLogger.error(`Login attempt failed: ${error.message}`);
            }
        ...
        if (this.profile) {
            lunaLogger.log("Twitter user ID:", this.profile.id);
            lunaLogger.log(
                "Twitter loaded:",
    ...
    async fetchOwnPosts(count: number): Promise<Tweet[]> {
        lunaLogger.debug("fetching own posts");
    ...
    async fetchHomeTimeline(count: number): Promise<Tweet[]> {
        lunaLogger.debug("fetching home timeline");
        const homeTimeline = await this.twitterClient.fetchHomeTimeline(
            count,
            []
        );

        lunaLogger.debug(homeTimeline, { depth: Infinity });
        ...
        //lunaLogger.debug("process homeTimeline", processedTimeline);
        return processedTimeline;
    }

    async fetchTimelineForActions(count: number): Promise<Tweet[]> {
        lunaLogger.debug("fetching timeline for actions");
        const homeTimeline = await this.twitterClient.fetchHomeTimeline(
            count,
            []
        );
    ...
    async fetchSearchTweets(
        ...
        try {
            ...
            try {
                ...
                lunaLogger.error("Error fetching search tweets:", error);
                return { tweets: [] };
            }
        } catch (error) {
            lunaLogger.error("Error fetching search tweets:", error);
    ...
    private async populateTimeline() {
        lunaLogger.debug("populating timeline...");
        ...
        if (cachedTimeline) {
            ...
            if (someCachedTweetsExist) {
                ...
                for (const tweet of tweetsToSave) {
                    lunaLogger.log("Saving Tweet", tweet.id);
                    ...
                    lunaLogger.log("Creating memory for tweet", tweet.id);
    ...
    async saveRequestMessage(message: Memory, state: State) {
        if (message.content.text) {
            ...
            if (
                ...
                lunaLogger.debug("Message already saved", recentMessage[0].id);
            } else {
                await this.runtime.messageManager.createMemory({
                    ...message,
                    embedding: getEmbeddingZeroVector(),
